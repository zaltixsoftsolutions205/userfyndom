...
// app/api/PricingService.ts - UPDATED FOR REAL API RESPONSE
import ApiClient from './ApiClient';
...
class PricingService {
  // Get hostel pricing (public endpoint) - UPDATED FOR REAL API
  async getHostelPricing(hostelId: string): Promise<PricingResponse> {
    try {
      console.log(`📊 Fetching pricing for hostel: ${hostelId}`);
  ...
  parsePricingData(apiResponse: PricingResponse): CombinedPricingData {
    const combined: CombinedPricingData = {};
    ...
    // Fill in pricing data from API
    if (apiResponse.pricing) {
      Object.keys(apiResponse.pricing).forEach(sharingType => {
        const typeKey = sharingType.toLowerCase();
        
        if (combined[typeKey]) {
          const pricing = apiResponse.pricing[sharingType];
          
          if (pricing.daily && pricing.daily > 0) {
            combined[typeKey].daily = {
              price: pricing.daily,
              currency: 'INR'
            };
            combined[typeKey].isAvailable = true;
          }
          
          if (pricing.monthly && pricing.monthly > 0) {
            combined[typeKey].monthly = {
  ...
  calculateStartingPrice(pricingData: CombinedPricingData): number {
    ...
    Object.keys(pricingData).forEach(type => {
      ...
      if (pricingData[type].daily?.price && pricingData[type].daily.price > 0) {
        // Convert daily to monthly (approx)
        prices.push(pricingData[type].daily.price * 30);
      }
    });
